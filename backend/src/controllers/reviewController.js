...
export const getAllReviews = (req, res) => {
  ...
  const query = `
    SELECT r.id, r.text, u.name AS user_name, r.rating
    FROM reviews r 
...
export const addReview = async (req, res) => {
  ...
  const query =
    'INSERT INTO reviews (book_id, user_id, text, rating) VALUES (?, ?, ?, ?)'
  const results = await db.query(query, [bookId, userId, review, rating])
